...
- **文件去重** — 按内容 Hash 查找重复文件，递归扫描文件夹，支持预览重复文件列表，可选择删除规则（按文件名/时间排序），删除前确认
...
- `plugins/pdf_merger.py` — PDF 合并工具，支持拖拽调整顺序
- `plugins/pdf_splitter.py` — PDF 拆分工具，支持拆分为图片或单页 PDF
- `plugins/file_deduplicator.py` — 文件去重工具，按内容 Hash 查找重复文件，支持多种删除规则
...
### 文件处理
- [x] PDF 拆分（拆成多张图片或单页 PDF）
- [x] PDF 合并（多个 PDF 合并为一个）
- [ ] 文件批量重命名（支持正则、序号、日期规则）
- [x] 文件去重（按内容 hash 查找重复文件）
