...
def main():
    ...
    app_bundle_path = Path("dist") / app_bundle_name
    ...
    if not app_bundle_path.exists():
        print(f"✗ 未找到目标应用包: {app_bundle_name}")
        # 列出 dist 目录中的所有文件供参考
        dist_dir = Path('dist')
        if dist_dir.exists():
    ...
    print("macOS 打包完成!")
    print("="*60)
    print(f"\n应用包位置: {app_bundle_path.absolute()}")
    print("\n测试建议:")
    print("  1. 运行应用，检查是否能正常启动")
    print("  2. 右键应用 → 显示简介，查看版本信息")
    print("  3. 测试所有功能模块:")
